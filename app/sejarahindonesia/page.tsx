...
export default function SejarahIndonesia() {
  ...
  const selectedItem = selectedId ? pembahasan.find((item) => item.id === selectedId) : null;

  return (
    <main className="w-full min-h-screen bg-[rgb(238,238,238)] px-4 md:px-10 py-8 scroll-smooth">
      <motion.h1
      ...
      {/* Grid untuk Kartu-kartu Ringkas */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {pembahasan.map((item) => (
          <motion.div
            ...
            className="bg-white rounded-xl shadow-lg cursor-pointer hover:shadow-2xl transition-shadow duration-300"
            whileHover={{ y: -5 }}
          >
            ...
            <div className="p-4">
              <h2 className="text-lg font-bold text-[rgb(48,56,65)] mb-2 truncate">{item.title}</h2>
      ...
      {/* Tampilan Detail Saat Kartu Diklik (Modal) */}
      <AnimatePresence>
        {selectedItem && (
          <motion.div
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-4xl max-h-[90vh] bg-white rounded-2xl shadow-2xl flex flex-col md:flex-row"
            >
              ...
              <div className="p-6 flex-grow overflow-y-auto">
                <motion.h2 className="text-2xl font-bold text-[rgb(48,56,65)] mb-4">{selectedItem.title}</motion.h2>
                <motion.p 
                  className="text-base text-[rgb(58,71,80)] leading-relaxed"
              ...
              <motion.button
                onClick={() => setSelectedId(null)}
                className="absolute top-3 right-3 bg-white/50 rounded-full p-1.5 hover:bg-white/80 transition-colors"
                aria-label="Tutup"
              >
